...
### Requirement: 搜索响应元信息
搜索响应 SHALL 包含：原始查询回显、查询向量前 5 维预览、嵌入模型名称、向量维度、top_k、总搜索向量数、涉及文档数、搜索耗时（毫秒）。

#### Scenario: 响应元信息完整性
- **WHEN** 搜索成功完成
- **THEN** 响应包含 query、query_embedding_preview、embedding_model、embedding_dimensions、top_k、total_searched、searched_document_count、search_time_ms 字段，且 search_time_ms 为正值
